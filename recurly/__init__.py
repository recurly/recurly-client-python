...
USER_AGENT = "recurly-python/%s; python %s; %s" % (
    recurly.__version__,
    recurly.__python_version__,
    recurly.resource.ssl.OPENSSL_VERSION,
)
...
def cache_rate_limit_headers(resp_headers):
    try:
        recurly.cached_rate_limits = {
            ...
            "resets_at": datetime.utcfromtimestamp(
                int(resp_headers["x-ratelimit-reset"])
            ),
        }
    except:
...
class Account(Resource):
    ...
    def verify(self, gateway_code=None):
        ...
        if gateway_code:
            ...
            response = self.http_request(
                url, "POST", body, {"content-type": "application/xml; charset=utf-8"}
            )
        else:
            response = self.http_request(url, "POST")
    ...
    def update_billing_info(self, billing_info):
        ...
        response = billing_info.http_request(
            url, "PUT", billing_info, {"content-type": "application/xml; charset=utf-8"}
        )
        if response.status == 200:
            pass
...
class DunningCampaign(Resource):
    ...
    def bulk_update(self, id, plan_codes):
        ...
        self.http_request(
            url, "PUT", body, {"content-type": "application/xml; charset=utf-8"}
        )
...
class GiftCard(Resource):
    ...
    def redeem(self, account_code):
        ...
        if hasattr(self, "_url"):
            ...
            url = urljoin(
                recurly.base_uri(), self.collection_path + "/" + redemption_path
            )
    ...
    def to_element(self, root_name=None):
        ...
        if "redemption_code" not in self.__dict__:  # not already included
            try:
                ...
                elem.append(self.element_for_value("redemption_code", redemption_code))
...
class Coupon(Resource):
    ...
    def value_for_element(cls, elem):
        ...
        if (
            elem is None
            or elem.tag not in excludes
            or elem.attrib.get("type") != "array"
        ):
...
class Invoice(Resource):
    ...
    def refund_amount(self, amount_in_cents, refund_options={}):
        ...
        amount_element = self._refund_open_amount_xml(amount_in_cents, refund_options)
    ...
    def refund(self, adjustments, refund_options={}):
        ...
        adjustments_element = self._refund_line_items_xml(adjustments, refund_options)
        return self._create_refund_invoice(adjustments_element)

    def _refund_open_amount_xml(self, amount_in_cents, refund_options):
        elem = ElementTreeBuilder.Element(self.nodename)
        elem.append(Resource.element_for_value("amount_in_cents", amount_in_cents))
...
class Purchase(Resource):
    ...
    def capture(self, transaction_uuid):
        ...
        """
        return self.__request_invoice(
            self.collection_path + "/transaction-uuid-" + transaction_uuid + "/capture"
        )
    ...
    def cancel(self, transaction_uuid):
        ...
        """
        return self.__request_invoice(
            self.collection_path + "/transaction-uuid-" + transaction_uuid + "/cancel"
        )
...
class Subscription(Resource):
    ...
    def postpone(self, next_bill_date, bulk=False):
        ...
        url = urljoin(
            self._url,
            "/postpone?next_bill_date="
            + next_bill_date.isoformat()
            + "&bulk="
            + str(bulk).lower(),
        )
        self.put(url)
    ...
    def pause(self, remaining_pause_cycles):
        ...
        elem = ElementTreeBuilder.Element(self.nodename)
        elem.append(
            Resource.element_for_value("remaining_pause_cycles", remaining_pause_cycles)
        )
        body = ElementTree.tostring(elem, encoding="UTF-8")
    ...
    def convert_trial(self, three_d_secure_action_result_token_id=None):
        ...
        if not three_d_secure_action_result_token_id == None:
            ...
            token = ElementTreeBuilder.SubElement(
                billing_info, "three_d_secure_action_result_token_id"
            )
...
class PlanRampInterval(Resource):
    ...
    attributes = {"starting_billing_cycle", "unit_amount_in_cents"}
...
class SubscriptionAddOn(Resource):
    ...
    _classes_for_nodename = {"percentage_tier": SubAddOnPercentageTier, "tier": Tier}
...
class ExportDate(Resource):
    ...
    def files(self, date):
        ...
        """
        url = urljoin(
            recurly.base_uri() + self.collection_path, "/%s/export_files" % date
        )
