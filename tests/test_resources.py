...
class TestResources(RecurlyTest):
    def test_authentication(self):
        ...
        try:
            ...
            self.fail(
                "Updating account with invalid email address did not raise a ValidationError"
            )
    ...
    def test_cached_response_headers(self):
        ...
        self.assertEqual(
            recurly.cached_rate_limits["resets_at"], datetime(2017, 2, 2, 19, 46)
        )
    ...
    def test_purchase(self):
        ...
        def create_purchase():
            return Purchase(
                ...
                subscriptions=[recurly.Subscription(plan_code="gold")],
                adjustments=[
                    recurly.Adjustment(
                        unit_amount_in_cents=1000, description="Item 1", quantity=1
                    ),
                    recurly.Adjustment(
                        unit_amount_in_cents=2000, description="Item 2", quantity=2
                    ),
                ],
            )
        ...
        with self.mock_request("purchase/invoiced.xml"):
            ...
            self.assertIsInstance(
                collection.charge_invoice.line_items[0].shipping_address,
                ShippingAddress,
            )
        ...
        with self.mock_request("purchase/captured.xml"):
            captured_collection = create_purchase().capture(
        ...
        with self.mock_request("purchase/cancelled.xml"):
            cancelled_collection = create_purchase().cancel(
                "40625fdb0d71f87624a285476ba7d73d"
            )
    ...
    def test_account(self):
        ...
        with self.mock_request("account/created.xml"):
            account.save()
        self.assertEqual(
            account._url, urljoin(recurly.base_uri(), "accounts/%s" % account_code)
        )
        self.assertEqual(account.vat_number, "444444-UK")
        self.assertEqual(account.vat_location_enabled, True)
        self.assertEqual(account.cc_emails, "test1@example.com,test2@example.com")
        ...
        self.assertEqual(
            same_account._url, urljoin(recurly.base_uri(), "accounts/%s" % account_code)
        )
        ...
        processing_prepayment_balance = (
            account_balance.processing_prepayment_balance_in_cents
        )
    ...
    def test_account_custom_fields(self):
        ...
        account = Account(
            account_code=account_code,
            custom_fields=[CustomField(name="field_1", value="my field value")],
        )
    ...
    def test_account_hierarchy(self):
        ...
        account = Account(
            account_code=account_code, parent_account_code="parent-account"
        )
        ...
        try:
            ...
            with self.mock_request("add-on/need-amount.xml"):
                try:
                    ...
                    self.fail(
                        "Creating a plan add-on without an amount did not raise a ValidationError"
                    )
        ...
        try:
            add_on = AddOn(
                ...
                tiers=[
                    ...
                    recurly.Tier(unit_amount_in_cents=recurly.Money(USD=800)),
                ],
            )
                    ...
                    recurly.CurrencyPercentageTier(
                        ...
                        tiers=[
                            recurly.PercentageTier(
                                ending_amount_in_cents=20000, usage_percentage="20"
                            ),
                            recurly.PercentageTier(
                                ending_amount_in_cents=40000, usage_percentage="25"
                            ),
                            recurly.PercentageTier(usage_percentage="30"),
                        ],
                    )
                ],
            )
    ...
    def test_item_backed_add_on(self):
        ...
        try:
            add_on = AddOn(item_code=item_code, unit_amount_in_cents=Money(500))
    ...
    def test_charge(self):
        ...
        try:
            ...
            with self.mock_request(
                "adjustment/account-has-item-backed-adjustments.xml"
            ):
                charges = account.adjustments()
            self.assertEqual(len(charges), 1)
            same_charge = charges[0]
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    self.assertFalse(
                        plan_coupon.has_unlimited_redemptions_per_account()
                    )
    ...
    def test_invoice_refund(self):
        ...
        with self.mock_request("invoice/line-item-refunded.xml"):
            line_items = [
                {"adjustment": invoice.line_items[0], "quantity": 1, "prorate": False}
            ]
            options = {
                "refund_method": "credit_first",
                "credit_customer_notes": "Credit Customer Notes",
    ...
    def test_invoice_offline_payment(self):
        ...
        with self.mock_request("invoice/offline-payment.xml"):
            transaction = Transaction(
                ...
                description="Collected externally",
            )
    ...
    def test_invoice_update(self):
        ...
        with self.mock_request("invoice/update-invoice.xml"):
            ...
            invoice.vat_reverse_charge_notes = (
                "can't be changed when invoice was not a reverse charge"
            )
            invoice.net_terms = 1
        ...
        self.assertEqual(
            ...
            "School staff is not responsible for items left at Hogwarts School of Witchcraft and Wizardry.",
        )
    ...
    def test_plan_with_ramps(self):
        ...
        plan.ramp_intervals = [
            PlanRampInterval(
                starting_billing_cycle=1, unit_amount_in_cents=Money(USD=3000)
            ),
            PlanRampInterval(
                starting_billing_cycle=2, unit_amount_in_cents=Money(USD=4000)
            ),
        ]
    ...
    def test_preview_subscription_change(self):
        with self.mock_request("subscription/show.xml"):
            ...
            with self.mock_request("subscription/change-preview.xml"):
                ...
                sub.preview()
                self.assertTrue(
                    sub.invoice_collection.charge_invoice.line_items[0].amount_in_cents,
                    2000,
                )
    ...
    def test_subscribe_add_on(self):
        ...
        try:
            ...
            with self.mock_request(
                "subscribe-add-on/percentage-tiered-add-on-created.xml"
            ):
    ...
    def test_subscription_custom_fields(self):
        ...
        sub = Subscription(
            ...
            account=Account(
                ...
                custom_fields=[
                    CustomField(
                        name="my_account_field",
                        value="here is the account value you seek",
                    )
                ],
            ),
            custom_fields=[
                CustomField(name="my_sub_field", value="definitely sub value")
            ],
        )
    ...
    def test_export_date_files(self):
        ...
        self.assertEqual(
            export_date_files[0].name, "churned_subscriptions_v2_expires.csv.gz"
        )
    ...
    def test_export_date_files_download_information(self):
        ...
        with self.mock_request(
            "export-date-files/export-date-file-download-information.xml"
        ):
            export_date_file_download_information = export_date_files[
                0
            ].download_information()

        self.assertEqual(
            export_date_file_download_information.expires_at.strftime(
                "%Y-%m-%d %H:%M:%S"
            ),
            "2019-05-09 14:00:00",
        )
        self.assertEqual(
            export_date_file_download_information.download_url,
